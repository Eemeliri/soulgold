...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_ABILITY_CAPSULE, {   _,   _,   _,   _,   1,   1,   4,   5,   4,   5, } },
    { ITEM_ABILITY_PATCH,   {   _,   _,   _,   _,   _,   _,   1,   1,   4,   5, } },
...
static enum BattlerId GetBattlerForConcurrentStatAnimMessage(enum BattlerId battler, bool32 isStatChangeMessage)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (!IsBattlerMarkedForControllerExec(i) && IsBattlerAlive(i))
        {
            RecordConcurrentStatAnimMessageForTest();
            return i;
        }
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (GetCurrentExpCapType() == EXP_CAP_HARD && gBattleStruct->battlerExpReward != 0)
                    {
    ...
    case 4: // lvl up if necessary
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (gBattleResources->bufferB[expBattler][0] == CONTROLLER_TWORETURNVALUES && gBattleResources->bufferB[expBattler][1] == RET_VALUE_LEVELED_UP)
            {
                ...
                if (GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL) >= MAX_LEVEL)
...
static u32 ChangeStatBuffs(enum BattlerId battler, s8 statValue, enum Stat statId, union StatChangeFlags flags, u32 stats, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        bool32 moldBreakerIgnoresStatLossAbility = battler != gBattlerAttacker
                                                && !hasAbilityShield
                                                && !SearchTraits(battlerTraits, ABILITY_FULL_METAL_BODY)
                                                && (gBattleStruct->moldBreakerActive
                                                 || HasMoldBreakerTypeAbility(gBattlerAttacker)
        ...
        else if ((BattlerHasHeldItemEffect(battler, HOLD_EFFECT_CLEAR_AMULET, TRUE)
               || (!moldBreakerIgnoresStatLossAbility && (CanBattlerPreventStatLoss(battler)
        ...
        else if ((index = IsFlowerVeilProtected(battler)) && !flags.certain)
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
                ...
                && !moldBreakerIgnoresStatLossAbility
...
static void Cmd_settailwind(void)
{
    ...
    if (!(gSideStatuses[side] & SIDE_STATUS_TAILWIND))
    {
        ...
        gSideTimers[side].tailwindTimer = (BattlerHasTrait(gBattlerAttacker, ABILITY_WINDCALLER) ? 6 : (GetConfig(B_TAILWIND_TURNS) >= GEN_5 ? 4 : 3));
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
    else
    {
...
void BS_SaveBattlerItem(void)
{
    ...
    if (itemslot == MAX_MON_ITEMS && gLastItemSlot < MAX_MON_ITEMS)
        itemslot = gLastItemSlot;
