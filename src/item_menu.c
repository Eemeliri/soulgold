...
void GoToBagMenu(u8 location, u8 pocket, MainCallback exitCallback)
{
    ...
    else
    {
        ...
        if (gBagPosition.location == ITEMMENULOCATION_BERRY_TREE ||
            ...
            gBagPosition.location == ITEMMENULOCATION_CHOOSE_ITEM ||
...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y)
{
    if (itemIndex != LIST_CANCEL)
    {
        ...
        if (gBagPosition.pocket != POCKET_KEY_ITEMS
         ...
         && !IsItemInfiniteHold(itemSlot.itemId))
        {
            // Print item quantity
...
static bool8 IsRegisteredKeyItemUsable(enum Item itemId)
{
    return itemId != ITEM_NONE
        && CheckBagHasItem(itemId, 1) == TRUE
...
static u32 CountRegisteredItems(void)
{
    ...
    if (count == 0)
        gSaveBlock1Ptr->registeredItemCompat = ITEM_NONE;
    else if (!IsRegisteredKeyItemUsable(gSaveBlock1Ptr->registeredItemCompat))
        gSaveBlock1Ptr->registeredItemCompat = firstUsableItem;
...
#define tSavedDispcnt data[13]
#define tSavedWinout data[14]
#define tSelectedSprite data[15]
...
static void FreeKeyItemWheelGfx(s16 *data)
{
    ...
    SetGpuReg(REG_OFFSET_DISPCNT, tSavedDispcnt);
    SetGpuReg(REG_OFFSET_WINOUT, tSavedWinout);
}
...
static void Task_KeyItemWheel(u8 taskId)
{
    ...
    case 0:
    {
        tSavedDispcnt = GetGpuReg(REG_OFFSET_DISPCNT);
        tSavedWinout = GetGpuReg(REG_OFFSET_WINOUT);
...
static void PrintPocketNames(const u8 *pocketName1, const u8 *pocketName2)
{
    ...
    fontId = GetFontIdToFit(pocketName1, FONT_NORMAL, 0, 60);
    offset = GetStringCenterAlignXOffset(fontId, pocketName1, 0x40);
    BagMenu_Print(windowId, fontId, pocketName1, offset, 1,
                  0, 0, TEXT_SKIP_DRAW, COLORID_POCKET_NAME);

    if (pocketName2)
    {
        fontId = GetFontIdToFit(pocketName2, FONT_NORMAL, 0, 60);
        offset = GetStringCenterAlignXOffset(fontId, pocketName2, 0x40);
        BagMenu_Print(windowId, fontId, pocketName2, offset + 0x40, 1,
                      0, 0, TEXT_SKIP_DRAW, COLORID_POCKET_NAME);
    }

    CpuCopy32((u8 *)GetWindowAttribute(windowId, WINDOW_TILE_DATA),
