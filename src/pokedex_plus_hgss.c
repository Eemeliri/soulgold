...
static const u8 sText_Stats_Defense[] = _("Ded");
...
static const u16 sPokedexPlusHGSS_Interface_dark_Pal[] = INCBIN_U16("graphics/pokedex/hgss/palette_interface_dark.gbapal");
...
static void AddPokedexListItem(enum NationalDexOrder dexNum);
static bool8 TrySelectPokedexListDexNum(enum NationalDexOrder dexNum);
static void TogglePokedexListView(void);
static u16 GetFilteredPokedexListSelection(void);
...
static const u16 sStatBarPalette[16] = {
    ...
    [COLOR_ID_FONT_SHADOW] = RGB(10, 10, 12),
#else
...
static const struct SpritePalette sInterfaceSpritePalette[] =
{
    ...
    {sPokedexPlusHGSS_Interface_dark_Pal, TAG_DEX_INTERFACE},
    {0}
};
...
static void Task_ClosePokedex(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        sLastSelectedPokemon = GetFilteredPokedexListSelection();
        sPokeBallRotation = sLastSelectedPokemon * 16 + POKEBALL_ROTATION_TOP;
...
static void LoadPokedexBgPalette(bool8 isSearchResults)
{
    if (!HGSS_DARK_MODE)
    {
        if (isSearchResults == TRUE)
            LoadPalette(sPokedexPlusHGSS_SearchResults_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_SearchResults_Pal) - PLTT_SIZEOF(1));
        else if (!IsNationalPokedexEnabled())
            LoadPalette(sPokedexPlusHGSS_Default_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_Default_Pal) - PLTT_SIZEOF(1));
        else
            LoadPalette(sPokedexPlusHGSS_National_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_National_Pal) - PLTT_SIZEOF(1));
    }
    else
    {
        if (isSearchResults == TRUE)
            LoadPalette(sPokedexPlusHGSS_SearchResults_dark_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_SearchResults_dark_Pal) - PLTT_SIZEOF(1));
        else if (!IsNationalPokedexEnabled())
            LoadPalette(sPokedexPlusHGSS_Default_dark_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_Default_dark_Pal) - PLTT_SIZEOF(1));
        else
            LoadPalette(sPokedexPlusHGSS_National_dark_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_National_dark_Pal) - PLTT_SIZEOF(1));
    }
...
#define temp_dexNum     vars[2]
    ...
    default:
    case DEX_MODE_HOENN:
        temp_dexCount = JOHTO_DEX_COUNT - 1;
    ...
    case ORDER_NUMERICAL:
        for (i = 0; i < temp_dexCount; i++)
        {
            ...
            if (sPokedexView->showUnseen || GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN))
                AddPokedexListItem(temp_dexNum);
        }
        break;
    case ORDER_ALPHABETICAL:
        for (i = 0; i < NATIONAL_DEX_COUNT; i++)
        {
            ...
            if ((!temp_isHoennDex || NationalToJohtoOrder(temp_dexNum) != 0)
             && (sPokedexView->showUnseen || GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN)))
                AddPokedexListItem(temp_dexNum);
        }
        break;
    case ORDER_HEAVIEST:
        for (i = NATIONAL_DEX_COUNT - 1; i >= 0; i--)
        {
            ...
            if ((!temp_isHoennDex || NationalToJohtoOrder(temp_dexNum) != 0)
             && (sPokedexView->showUnseen || GetSetPokedexFlag(temp_dexNum, FLAG_GET_CAUGHT)))
                AddPokedexListItem(temp_dexNum);
        }
        break;
    case ORDER_LIGHTEST:
...
static void AddPokedexListItem(enum NationalDexOrder dexNum)
{
    struct PokedexListItem *item = &sPokedexView->pokedexList[sPokedexView->pokemonListCount++];

    item->dexNum = dexNum;
    item->seen = GetSetPokedexFlag(dexNum, FLAG_GET_SEEN);
    item->owned = GetSetPokedexFlag(dexNum, FLAG_GET_CAUGHT);
}
...
static bool8 IsPokedexListItemShownInFilteredView(const struct PokedexListItem *item)
{
    if (sPokedexView->dexOrder == ORDER_NUMERICAL || sPokedexView->dexOrder == ORDER_ALPHABETICAL)
        return item->seen;
    else
        return item->owned;
}
...
static void TogglePokedexListView(void)
{
    enum NationalDexOrder selectedDexNum = sPokedexView->pokedexList[sPokedexView->selectedPokemon].dexNum;
    enum NationalDexOrder fallbackDexNum = NATIONAL_DEX_NONE;
    u16 oldListCount = sPokedexView->pokemonListCount;
    u16 oldSelection = sPokedexView->selectedPokemon;
    u16 lastSelectedPokemon = sLastSelectedPokemon;
    u8 pokeBallRotation = sPokeBallRotation;
    u16 distance;

    sPokedexView->showUnseen ^= TRUE;

    if (!sPokedexView->showUnseen)
    {
        for (distance = 0; distance < oldListCount; distance++)
        {
            if (distance <= oldSelection
             && IsPokedexListItemShownInFilteredView(&sPokedexView->pokedexList[oldSelection - distance]))
            {
                fallbackDexNum = sPokedexView->pokedexList[oldSelection - distance].dexNum;
                break;
            }
            if (oldSelection + distance < oldListCount
             && IsPokedexListItemShownInFilteredView(&sPokedexView->pokedexList[oldSelection + distance]))
            {
                fallbackDexNum = sPokedexView->pokedexList[oldSelection + distance].dexNum;
                break;
    ...
    CreatePokedexList(sPokedexView->dexMode, sPokedexView->dexOrder);
    sPokedexView->selectedPokemon = 0;
    if (!TrySelectPokedexListDexNum(selectedDexNum) && fallbackDexNum != NATIONAL_DEX_NONE)
        TrySelectPokedexListDexNum(fallbackDexNum);
    sPokedexView->pokeBallRotation = sPokedexView->selectedPokemon * 16 + POKEBALL_ROTATION_TOP;
    sLastSelectedPokemon = lastSelectedPokemon;
    sPokeBallRotation = pokeBallRotation;
    CreateMonSpritesAtPos(sPokedexView->selectedPokemon, 0xE);
    CreateStatBars(&sPokedexView->pokedexList[sPokedexView->selectedPokemon]);
}
...
static u16 GetFilteredPokedexListSelection(void)
{
    u16 selection = sPokedexView->selectedPokemon;
    u16 filteredSelection = 0;
    u16 distance;
    u16 i;

    if (!sPokedexView->showUnseen || sPokedexView->pokemonListCount == 0)
        return selection;

    if (!IsPokedexListItemShownInFilteredView(&sPokedexView->pokedexList[selection]))
    {
        for (distance = 1; distance < sPokedexView->pokemonListCount; distance++)
        {
            if (distance <= selection
             && IsPokedexListItemShownInFilteredView(&sPokedexView->pokedexList[selection - distance]))
            {
                selection -= distance;
                break;
            }
            if (selection + distance < sPokedexView->pokemonListCount
             && IsPokedexListItemShownInFilteredView(&sPokedexView->pokedexList[selection + distance]))
            {
                selection += distance;
                break;
    ...
    {
        if (IsPokedexListItemShownInFilteredView(&sPokedexView->pokedexList[i]))
            filteredSelection++;
    }
...
static void CreateMonDexNum(u16 entryNum, u8 left, u8 top, u16 unused)
{
    ...
    if ((sPokedexView->dexMode == DEX_MODE_HOENN ? JOHTO_DEX_COUNT - 1 : NATIONAL_DEX_COUNT) > 999)
    {
...
//************************************
#if HGSS_DARK_MODE
static const u8 sPokedexWhiteTextColor[] = {TEXT_COLOR_TRANSPARENT, TEXT_DYNAMIC_COLOR_6, TEXT_COLOR_LIGHT_GRAY};
#else
static const u8 sPokedexWhiteTextColor[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_WHITE, TEXT_DYNAMIC_COLOR_6};
#endif
...
static void PrintInfoScreenTextWhite(const u8* str, u8 left, u8 top)
{
    AddTextPrinterParameterized4(0, FONT_NORMAL, left, top, 0, 0, sPokedexWhiteTextColor, TEXT_SKIP_DRAW, str);
}
static void PrintInfoScreenTextSmall(const u8* str, u8 fontId, u8 left, u8 top)
{
    u8 color[3];
...
{
    AddTextPrinterParameterized4(0, 0, left, top, 0, 0, sPokedexWhiteTextColor, 0, str);
}
//Stats screen
...
{
    AddTextPrinterParameterized4(windowId, 0, left, top, 0, 0, sPokedexWhiteTextColor, 0, str);
}
...
static void PrintMonInfo(u32 num, u32 value, u32 owned, u32 newEntry)
{
    ...
    const u8 *category;
    const u8 *description;
    u8 digitCount = ((value == 0 ? JOHTO_DEX_COUNT - 1 : NATIONAL_DEX_COUNT) > 999) ? 4 : 3;
...
//*                                  *
//************************************
static const u8 sNavigationTextColor[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_WHITE, TEXT_COLOR_DARK_GRAY};
...
static void StatsPage_PrintNavigationButtons(void)
{
    ...
    u8 y = 0;
    if (!HGSS_DECAPPED)
        AddTextPrinterParameterized3(WIN_STATS_NAVIGATION_BUTTONS, 0, x, y, sNavigationTextColor, 0, sText_Stats_Buttons);
    else
        AddTextPrinterParameterized3(WIN_STATS_NAVIGATION_BUTTONS, 0, x, y, sNavigationTextColor, 0, sText_Stats_Buttons_Decapped);
...
static void EvoFormsPage_PrintNavigationButtons(void)
{
    ...
    FillWindowPixelBuffer(WIN_NAVIGATION_BUTTONS, PIXEL_FILL(0));
        ...
        {
            if (!HGSS_DECAPPED)
                AddTextPrinterParameterized3(WIN_NAVIGATION_BUTTONS, 0, x+9, y, sNavigationTextColor, 0, sText_EVO_Buttons_PE);
            else
                AddTextPrinterParameterized3(WIN_NAVIGATION_BUTTONS, 0, x+9, y, sNavigationTextColor, 0, sText_EVO_Buttons_Decapped_PE);
        }
        else if (sPokedexView->selectedScreen == FORMS_SCREEN)
        {
            ...
            {
                if (!HGSS_DECAPPED)
                    AddTextPrinterParameterized3(WIN_NAVIGATION_BUTTONS, 0, x, y, sNavigationTextColor, 0, sText_FORMS_Buttons_Submenu_PE);
                else
                    AddTextPrinterParameterized3(WIN_NAVIGATION_BUTTONS, 0, x, y, sNavigationTextColor, 0, sText_FORMS_Buttons_Submenu_Decapped_PE);
            }
            else
            {
                if (!HGSS_DECAPPED)
                    AddTextPrinterParameterized3(WIN_NAVIGATION_BUTTONS, 0, x, y, sNavigationTextColor, 0, sText_FORMS_Buttons_PE);
                else
                    AddTextPrinterParameterized3(WIN_NAVIGATION_BUTTONS, 0, x, y, sNavigationTextColor, 0, sText_FORMS_Buttons_Decapped_PE);
            }
        }
        ...
        else
        {
            ...
            for (u32 j = 0; evolutions[i].params != NULL && evolutions[i].params[j].condition != CONDITIONS_END; j++)
            {
                ...
                case IF_NOT_REGION:
                {
                    ...
                    case REGION_HOENN: StringAppend(gStringVar4, COMPOUND_STRING("Johto")); break;
...
static void Task_LoadSearchMenu(u8 taskId)
{
    ...
    case 0:
        if (!gPaletteFade.active)
        {
            ...
            if (!HGSS_DARK_MODE)
                LoadPalette(sPokedexPlusHGSS_MenuSearch_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_MenuSearch_Pal) - PLTT_SIZEOF(1));
            else
                LoadPalette(sPokedexPlusHGSS_MenuSearch_dark_Pal + 1, BG_PLTT_ID(0) + 1, sizeof(sPokedexPlusHGSS_MenuSearch_dark_Pal) - PLTT_SIZEOF(1));
            gMain.state = 1;
        }
        break;
    case 1:
